...
| §A-4 | 格闘方向倍率・連撃補正・ヘビーアタック | `directionMul` / `byDirection` | **項の存在は確定・数値未確定** → 暫定 front/side/back=1.0・単発のみ。要追加調査(atwiki pages/83.html 全文) |
...
- 武器属性判定 = **カテゴリ節見出しで確定**（色は CSS 由来で不可）。フォールバック=カラム構成→武器名。
- 集束/変形の複数威力モードは**式でなくデータ駆動**（武装表に別威力で載る）→ §4-2 `modes`。
