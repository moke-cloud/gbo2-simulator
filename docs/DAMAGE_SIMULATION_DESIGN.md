...
```jsonc
{
  ...
  "special": {
    ...
    "heavyAttack": false,         // ヘビーアタック対応（格闘）。倍率確定済(2026-06-11):
                                  //   Wiki機体ページ「格闘方向補正」表のHAカラム由来
                                  //   {mul:2.7, perHitMul:1.35, hits:2}（=「270%(135%x2)」）
                                  //   倍率カラム未記載機のみ true（未モデル警告表示）
...
- `directionMul`（§A-4）: **確定済(2026-06-10)** = N格/横格/下格の入力方向・標準 100%/75%/130%・武器別上書きあり（Wiki「格闘方向補正」表）。
  - **ヘビーアタックも確定(2026-06-11)**: 同表のHAカラムに機体別倍率（例 ガズエル 270%(135%x2)）。
    方向倍率と同段で適用・多段は1ヒットごと floor して hits 倍。緩衝材無効/シールド貫通は部位未モデルのため非考慮（UI警告）。
  - コンボ段（連撃補正）のみ未実装（▶後段。標準: 2撃目50%減/3撃目75%減、格闘連撃制御LV3は増幅）。
...
| §A-4 | 格闘方向倍率・連撃補正・ヘビーアタック | `directionMul` / `byDirection` | **2026-06-10 ほぼ確定（Wiki構造で発見）**: 「格闘方向補正」の正体は**N格/横格/下格（入力方向）**であり正面/側面/背面ではない。Wiki武器ページに「格闘方向補正」表（格闘方向/標準倍率/本武器倍率）があり、**標準倍率 = N格100% / 横格75% / 下格130%**・武器ごとに上書きあり（例: 横格100%(50%x2)）。「連撃補正」表も同様に存在（例: 1撃目100%/2撃目50%）。→ エンジンは標準値で実装・武器別上書きはパーサで取得。**ヘビーアタック倍率も確定(2026-06-11)**: 同表のHAカラム（rowspanで全方向共通・「270%(135%x2)」形式＝総倍率(1ヒット倍率×ヒット数)）。パーサが special.heavyAttack={mul,perHitMul,hits} へ格納し、エンジンは方向倍率と同段で適用（多段は1ヒットごとfloor→hits倍）。倍率カラム未記載機は true のまま未モデル警告。連撃補正のみ▶後段（標準: 2撃目50%減/3撃目75%減） |
...
- 武器属性判定 = ~~カテゴリ節見出し~~ → **`table_weapon_{shell|beam|close|etc}` の div id で確定**（1-D実装済・見出しより頑健）。
  ※「その他」の実suffixは `etc`（`other` は実在せず）。2026-06-11 修正＝閃光弾/スモーク/［速射］［照射］系 94表を回収。
