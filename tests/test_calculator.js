...
// ===== 拡張: よろけ蓄積閾値（ダメージコントロール） =====
section('extractSkillEffects: 蓄積よろけ閾値（ダメージコントロール）');
{
  // 実データ: ザクⅢ改 LV1 のダメージコントロール LV2
  const dmgCtl = {
    name: 'ダメージコントロール', level: 'LV2',
    effect: '拘束効果のない攻撃を連続して受けた際に生じるよろけを発生しづらくする。・蓄積よろけまでの値が160%になる',
  };
  const effects = GBO2Calculator.extractSkillEffects(dmgCtl);
  const th = effects.find(e => e.category === 'stagger_threshold');
  assert('stagger_threshold が抽出される', !!th, true);
  assert('閾値160%', th.value, 160);
  assert('発動条件は常時（既定ON）', th.condition, '常時');
  assert('stagger（よろけ値圧縮）と誤判定しない', effects.some(e => e.category === 'stagger'), false);

  // LV1=130% / LV3=200% も同形式
  const lv1 = GBO2Calculator.extractSkillEffects({ effect: '蓄積よろけまでの値が130%になる' });
  assert('LV1: 130%', lv1.find(e => e.category === 'stagger_threshold').value, 130);
  const lv3 = GBO2Calculator.extractSkillEffects({ effect: '蓄積よろけまでの値が200%になる' });
  assert('LV3: 200%', lv3.find(e => e.category === 'stagger_threshold').value, 200);

  // 「大よろけ攻撃を受けた際、よろけ値＋20%」（特殊な被弾増加）は閾値・圧縮どちらにも誤検出しない
  const odd = GBO2Calculator.extractSkillEffects({ effect: '大よろけ攻撃を受けた際、よろけ値＋20%' });
  assert('よろけ値＋20%は閾値に誤検出しない', odd.some(e => e.category === 'stagger_threshold'), false);
  assert('よろけ値＋20%は圧縮に誤検出しない', odd.some(e => e.category === 'stagger'), false);

  // マニューバーアーマー等の「よろけ値をX%で計算」と共存した場合は両方抽出される
  const both = GBO2Calculator.extractSkillEffects({
    effect: '高速移動中に受けた攻撃のよろけ値を70%で計算する。蓄積よろけまでの値が130%になる',
  });
  assert('圧縮70%を抽出', both.find(e => e.category === 'stagger')?.value, 70);
  assert('閾値130%を抽出', both.find(e => e.category === 'stagger_threshold')?.value, 130);

  // 実質よろけ耐性の合成式（app.js updateSkillPanel と同一式）: 閾値 ÷ 圧縮率
  const resist = (cap, combined) => Math.round(cap / combined);
  assert('閾値160% × 圧縮なし → 耐性160%', resist(160, 1.0), 160);
  assert('閾値160% × 圧縮0.5 → 耐性320%', resist(160, 0.5), 320);
  assert('閾値100% × 圧縮0.8 → 耐性125%', resist(100, 0.8), 125);
}
